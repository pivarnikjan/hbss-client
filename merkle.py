...
class MerkleTree:
    ...
    def _generate_hashchain_keypairs(self, keynum):
        while keynum > 0:
            keynum -= 1
            newkey = diplomovka.Keypair(RNG=RNG)
    ...
    def select_unused_key(self, mark_used=True, force=False):
        ...
        try:
            if self.key_type == "hash chain":
                keypair = diplomovka.Keypair(private_seed = private_key)
            else:
                keypair = diplomovka.Keypair(keypair = private_key)
...
def runtests():
    ...
    # print(mytree._exportable_tree())
    # for layer in mytree._exportable_tree():
    #     for node in layer:
    #         print(node)
    #     print("\n")
    ...
    # [print(base64.b64encode(item)) for item in mytree.root_hash()]
